...
class CopilotCliProvider:
    ...
    def __init__(
        ...
        *,
        ...
        self._command = command or settings.copilot_cli_command
    ...
    async def execute(self, work_package: ApprovedWorkPackage) -> WorkerExecutionResult:
        ...
        if not argv:
            raise ProviderExecutionError("Copilot CLI command is not configured.")
        ...
        try:
            ...
            raise ProviderExecutionError("Copilot CLI command returned invalid JSON.") from exc

        if not isinstance(payload, dict):
            raise ProviderExecutionError("Copilot CLI command returned a non-object JSON payload.")
    ...
    def _build_dry_run_result(self, work_package: ApprovedWorkPackage) -> WorkerExecutionResult:
        ...
        return WorkerExecutionResult(
            ...
            execution_summary=(
                "Copilot CLI provider executed the approved work package in dry-run mode. "
                f"Configured command: {self._command}"
            ),
            known_risks=[
                "Dry-run provider did not invoke a real Copilot CLI session.",
            ],
        )
